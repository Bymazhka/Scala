...
object Compositions {
  ...
  def testCompose[A, B, C, D](f: A => B)
                             (g: B => C)
                             (h: C => D): A => D = h compose(g) compose(f) //создает новую функцию, которая является объединением других функций f(g(x))
  ...
  def testMapFlatMap[A, B, C, D](f: A => Option[B])
                                (g: B => Option[C])
                                (h: C => D): Option[A] => Option[D] = x => x flatMap (f) flatMap(g) map (h) //map - применяет функцию. flatMap объединяет результаты.
  ...
  def testForComprehension[A, B, C, D](f: A => Option[B])
                                      (g: B => Option[C])
                                      (h: C => D): Option[A] => Option[D] =  for{a <- _
                                                                                 b<- f(a)
                                                                                 c<- g(b)
                                                                                 } yield h(c)
}
